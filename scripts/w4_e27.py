#!/usr/bin/env python3
"""E27 fixed scientific runner and separately named legal-seen QA runner."""
...
def load_manifest(path=e.PREFLIGHT, *, root=e.ROOT):
    ...
    if manifest != e.make_manifest(root):
        raise ValueError("E27 frozen manifest changed")
    for seed in e.SEEDS:
...
def compare(current, baseline):
    result = {
        "composition": accepted._composition_comparison(baseline["composition"], current["composition"]),
    ...
    for split, rows in result["seen"].items():
        ...
        for row in rows:
            ...
            for state in a:
                ac = bool(a[state]["joint_final_correct"])
                bc = bool(b[state]["joint_final_correct"])
...
def compare_both(current, seed, *, root=e.ROOT):
    comparators = (
        ("E26_Wternary_A32", e.E26_RUN),
        ("E24_float", e.FLOAT_RUN),
    )
    return {
        label: compare(current, accepted._read_json(root / path / "report.json")["seeds"][str(seed)]["evaluation"])
        for label, path in comparators
    }
...
def costs(report):
    ...
    return {
        "attempted_updates": sum(r.get("attempted_updates", 0) for r in records),
        ...
        **{
            ...
            for phase in (
                "training_cost", "evaluation_cost", "reload_identity_cost",
...
def execute(out, manifest, batches, evaluator, *, qa, root=e.ROOT):
    """Shared mechanics; scientific execution supplies only registered inputs."""
    out = Path(out)
    ...
    if not qa and (
        len(batches) != e.UPDATES
        or old.batch_digest(batches) != manifest["full_stream_digest"]
        or old.target_digest(batches) != manifest["full_target_digest"]
    ):
        raise ValueError("scientific stream mismatch")
    out.mkdir(parents=True, exist_ok=True)
    accepted._atomic_json(out / "manifest.json", manifest, refuse=True)
    report = {
        "schema": e.SCHEMA, "qa": qa, "status": "running", "seeds": {},
        "cost_note": "Completed-forward costs exclude partial failed forwards; attempted workload is reported separately.",
    }
    try:
        for seed in e.SEEDS:
            record = {
                "status": "running", "completed_updates": 0, "attempted_updates": 0,
                "training_cost": zero_cost(), "evaluation_cost": zero_cost(),
                "reload_identity_cost": zero_cost(), "progress": [],
            }
            ...
            try:
                ...
                [handle.remove() for handle in active_hooks]
            ...
            record["predicates"] = (
                {"combined_conjunction": False, "primary_conjunction": False, "seen_prerequisite": False}
                if qa else accepted.predicates(evaluation["seen"], evaluation["composition"])
            )
            if qa:
                # Two genuine next updates start from identical saved optimizer/RNG state.
                next_rngs = []
...
def tiny_qa(out):
    """Real three-seed QA with one legal seen update and evaluation per seed."""
    batch = old.base_stream()[0][:2]
    models = {str(seed): e.build_initial_model(seed) for seed in e.SEEDS}
    manifest = {
        "schema": e.SCHEMA, "qa": True,
        "initial_digests": {seed: old.digest_state_dict(model) for seed, model in models.items()},
        "initial_rng_digests": {seed: old.digest_object(model.initial_rng_state) for seed, model in models.items()},
