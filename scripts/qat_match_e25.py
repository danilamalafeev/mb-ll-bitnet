#!/usr/bin/env python3
"""E25 fixed scientific runner and separately named legal-seen QA runner."""
...
def load_manifest(path=e.PREFLIGHT, *, root=e.ROOT):
    ...
    if manifest != e.make_manifest(root):
        raise ValueError('E25 frozen manifest changed')
    for seed in e.SEEDS:
...
def costs(report):
    ...
    return {'completed_updates': sum(r.get('completed_updates', 0) for r in records),
    ...
    try:
        for seed in e.SEEDS:
            ...
            if qa:
                ...
                baseline = accepted._read_json(root / e.FLOAT_RUN / 'report.json')['seeds'][str(seed)]['evaluation']
                record['paired_comparison'] = compare(evaluation, baseline)
